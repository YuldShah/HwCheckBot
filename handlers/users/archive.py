...
async def show_archive(message: types.Message, state: FSMContext):
    ...
    mexams = db.fetchall("SELECT title, idx FROM exams LEFT JOIN submissions ON exams.idx = submissions.exid AND submissions.userid = ? WHERE submissions.exid IS NULL AND exams.hide = 0", (message.from_user.id,))
    if mexams:
